...
| `timestamp` | integer | Time of the **WiFi scan** in Unix epoch milliseconds. This should _not_ be system/wallclock time of when the location request is sent, but the time of when the scan itself took place. For Android, this would be [ScanResult.timestamp](https://developer.android.com/reference/android/net/wifi/ScanResult#timestamp) (which also must be converted from elapsed time since system boot to epoch time).<br/>_NOTE: This value must be within 24 hours of real time, else an error response will be returned._ |
